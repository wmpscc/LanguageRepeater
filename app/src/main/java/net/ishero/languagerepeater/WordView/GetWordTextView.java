package net.ishero.languagerepeater.WordView;
...
import net.ishero.languagerepeater.R;
...
public class GetWordTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    private Map<String, Integer> highlightColorMap;
    ...
    private void setText() {
        ...
        //set highlight text
        setHighLightArraySpan(mSpannableString);
        //separate word
    ...
    private void setHighLightArraySpan(SpannableString spannableString) {
        if (TextUtils.isEmpty(highlightText)) {
            return;
        }
        String highTexts[] = highlightText.split(" ");
        Log.e("num_hword", "count: " + highTexts.length);
        for (String hText : highTexts) {
            int hIndex = mText.toString().indexOf(hText);
            while (hIndex != -1) {
                int color;
                if (highlightColorMap.get(hText) == null)
                    color = Color.BLACK;
                else color = highlightColorMap.get(hText);

                spannableString.setSpan(new ForegroundColorSpan(color), hIndex, hIndex + hText.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                hIndex += hText.length();
                hIndex = mText.toString().indexOf(hText, hIndex);
            }
        }
    ...
    public void setHighLightArrayColor(Map<String, Integer> color) {
